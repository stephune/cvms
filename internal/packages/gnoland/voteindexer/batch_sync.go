...
func (vidx *VoteIndexer) batchSync(lastIndexPointerHeight, newIndexPointerHeight int64) (
	...
	// init summary list
	blockSummaryList := make(map[int64]types.GnolandBlockSummary, 0)
	...
	// add -1 new index pointer height data// get current block for collecting last commit signatures
	{
		...
		blockHeight, blockTimestamp, blockProposerAddress, _, lastCommitBlockHeight, blockSignatures, err := api.GetGnolandBlock(vidx.CommonClient, height)
		if err != nil {
	...
	for height := startHeight; height <= endHeight; height++ {
		...
		go func(ch chan helper.Result) {
			...
			ch <- helper.Result{
				Item: types.GnolandBlockSummary{
					BlockHeight:           blockHeight,
					BlockTimeStamp:        blockTimestamp,
					BlockProposerAddress:  blockProposerAddress,
					LastCommitBlockHeight: lastCommitBlockHeight,
	...
	for r := range ch {
		if r.Success {
			item := r.Item.(types.GnolandBlockSummary)
	...
	for _, blockInfo := range blockSummaryList {
		...
		for _, validator := range blockInfo.GnolandBFTValidators {
	...
	if isNewValidator {
		newValidatorInfoList, err := api.MakeGnolandValidatorInfoList(vidx.CommonApp, vidx.ChainInfoID, newValidatorAddressMap)
		if err != nil {
			errors.Wrap(err, "failed to make validator info list")
		}
		vidx.Debugf("insert new gnoland validators: %d", len(newValidatorInfoList))
	...
	// vidx.Panicf("%v", blockSummaryList)
	...
	for height := startHeight; height <= endHeight; height++ {
		...
		tempValidatorVoteList, err := makeValidatorVoteList(
			...
			blockSummaryList[lastCommitHeight].GnolandBFTValidators,
			// current block data
...
func makeValidatorVoteList(
	...
	lastCommitValidators []types.GnolandBFTValidator,
	// current block data
	blockSignatures []types.GnolandSignature,
	...
	for idx, validator := range lastCommitValidators {
		...
		if blockSignatures[idx].Signature == "" {
			vml.Debugf(
